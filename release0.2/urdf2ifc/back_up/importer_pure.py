    ...
    for child in domFile.childNodes:
        if child.localName == 'robot':  # root tag
            ...
            if color is not None:
                    ...
                    ifcfile.createIfcRelAssociatesMaterial(ifcopenshell.guid.new(), owner_history, RelatedObjects=[entity],
...
'''
def create_ifc_rect(x1, y1, x2, y2, name, color=None, ifcFile):
    p1 = gp_Pnt(x1, y1, 0)
    p2 = gp_Pnt(x2, y2, 500)
    myBody = BRepPrimAPI_MakeBox(p1, p2).Shape()
    rep = ifcopenshell.geom.tesselate(ifcFile.schema, myBody, 1.)
