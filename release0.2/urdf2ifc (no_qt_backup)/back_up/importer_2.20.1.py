...
def convertUrdfContent(urdfContent, robotName=None):   #Convert a URDF content string into a ifc file
    ...
    for child in domFile.childNodes:
        if child.localName == 'robot':  # root tag
            ...
            #ifcFile = open(outputFile, 'w')     #write outputFile
            for link in linkList:
                print(link.inertia.ixx)

            # ifcTemplate = readTemplate()

            # tempFile = open(outputFile, 'wb') 
            # tempFile.write(ifcTemplate.encode()) 

            # ifcFile = ifcopenshell.open(tempFile)
            ...
            with open("equipment.pkl", "rb") as tf:
            ...
            for key in contour.keys():
                ...
                name = 'vav_4'
                _contours = contour[key]
                _bottom_height = 0.0
                _wall_height = 100.0
                # color = [0.0, 0.98, 0.99]
                color = [0.98, 0.0, 0.99]

                contours = scale * _contours[:, [1, 0]]
                ...
                # z_extrusion_placement = create_ifcaxis2placement(ifcfile, point=(1000.0, 1000.0, float(_bottom_height)))
                # z_solid = create_ifcextrudedareasolid(ifcfile,
                #                                     [(0.0, 0.0, 1000.0), (500.0, 0.0, 1000.0), (0.0, 500.0, 1000.0)],
